...
* [Data Wrangling with dplyr and tidyr](#data-wrangling-with-dplyr-and-tidyr)
  ...
  - [Pivoting Dataframe](#pivoting-dataframe)
    ...
    - [Wider Dataframe](#wider-dataframe)
  
    - [Longer Datafarme](#longer-datafarme)
...
* [Cleaning a Messy Data](#cleaning-a-messy-data) 
  ...
  - [Filter Out the Data with NS](#filter-out-the-data-withn-ns)

  - [Convert ND to Empty String](#convert-nd-to-empty-string)

  - [Remove Spacial Characters and Convert Data to Double](#remove-spacial-characters-and-convert-data-to-double)

  - [Replace the Values Below Detection Limits](#replace-the-values-below-detection-limits)
...
In this exercise  we will use two data sets: USA county level lung cancer mortality rates with risk factors from 1998 t0 2012 and USA  County FIPS Codes. All data set use in this exercise could be found [here](https://www.dropbox.com/sh/ctjv2eiifhfh2ts/AAAVXmWxPntD-O-5AE_YH3VHa?dl=0). 
...
```{r Sturcture of Data, message=FALSE, warning=FALSE}
str(df); 
```
<div style="margin-bottom:20px;">
</div>
...
```{r Get a glimpse of data, message=FALSE, warning=FALSE}
dplyr::glimpse(df)
```
...
```{r Inner Join-1, message=FALSE, warning=FALSE}
fips_df = dplyr::inner_join(fips, regions)
head(fips_df)
```

```{r Inner Join-2, message=FALSE, warning=FALSE}
mf = dplyr::inner_join(fips_df, df)
head(mf)
```
...
```{r Change column order}
mf<-dplyr::relocate(mf, REGION,   DIVISION, .after = FIPS)
head(mf)
```

Now explore regions names with **levels()**

```{r Factor levels, message=FALSE, warning=FALSE}
...
```{r Filter Data-0, message=FALSE, warning=FALSE}
mf.northeast<-mf %>% dplyr::filter(REGION == "Northeast")
...
```{r Filter Data-1, message=FALSE, warning=FALSE}
...
```{r Filter Data-2, warning=FALSE}
mf.north_mid_02<- mf %>% dplyr::filter(REGION == "Northeast" | REGION == "Midwest")
...
mf.ny<-mf %>% dplyr::filter(REGION == "Northeast" & STATE == "New York")
```


Following filters will select counties rows where `LBC_RATE` is greater than the global average of LBC - rate:

```{r Filter Data-3, message=FALSE, warning=FALSE}
mean.LBC <- mf %>% dplyr::filter(LBC_RATE > mean(LBC_RATE, na.rm = TRUE))
```
...
```{r Fliter Data, message=FALSE, warning=FALSE}
mean.LBC.2012 <- mf %>% dplyr::filter(LBC_RATE > mean(LBC_RATE, na.rm = TRUE) & Year ==2012)
```
...
```{r Filter Data-4, message=FALSE, warning=FALSE}
counties.a <- mf %>% dplyr::filter(grepl("^A", COUNTY))
head(counties.a)
```
<div style="margin-bottom:20px;">
</div>
...
```{r Select variable-1, message=FALSE, warning=FALSE}
LBC.df <- mf %>% dplyr::select(FIPS, Year, LBC_RATE)
head(LBC.df)
```

Following will create a dataframe with FIPS, Year all risk factors (SMOKING, POVERTY, PM25, NO2, SO2)

```{r Select Variables-2, message=FALSE, warning=FALSE}
riskfactors.df <- mf %>% dplyr::select(FIPS, Year, SMOKING:SO2)
head(riskfactors.df)
```
...
```{r Select Variable-3, message=FALSE, warning=FALSE}
riskfactors_01.df= mf %>% dplyr::select(-(REGION:Y), -LBC_RATE)
...
mf.new <- mf %>% 
        dplyr::rename("LBC_Mortality_Rate" = "LBC_RATE")
names(mf.new)
```
<div style="margin-bottom:20px;">
</div>
...
```{r Arrange accending, message=FALSE, warning=FALSE}
# arrange ascending order as default
mf.arrange.ac<- mf %>% dplyr::arrange(SMOKING) 
head(mf.arrange.ac$SMOKING)
```

```{r Arrange decending}
# arrange descending order
mf.arrange.de<- mf %>% dplyr::arrange(desc(SMOKING)) 
...
```{r Combined multiple functions with Pipes, message=FALSE, warning=FALSE}
LBC.NY <- mf %>% dplyr::select(FIPS, Year, STATE, LBC_RATE) %>%
                 dplyr::filter (STATE=="New York", Year == 2012)  %>%
                 dplyr::rename("LBC Mortality Rate"= LBC_RATE)
...
**pivot_longer()** and **pivot_wider()** from  **tidyr** package are very flexible, and can easily tidy a wide variety of non-tidy datasets. 
...
```{r Wider dataframe, message=FALSE, warning=FALSE}
LBC.wider<- mf %>% dplyr::select (FIPS, Year, STATE, COUNTY, LBC_RATE) %>%
                   tidyr::pivot_wider(names_from = Year, # column need to be wider
...
```{r Longer Dataframe, message=FALSE, warning=FALSE}
LBC.longer<- LBC.wider %>% 
              tidyr::pivot_longer(cols= c("1998", "1999", "2000", "2001", "2002",
...
```{r Mutate-1, message=FALSE, warning=FALSE}
mf.new<- mf %>% dplyr::mutate(logLBC = log10(LBC_RATE))
head(mf.new[9:15])
```
...
```{r Mutate-2, message=FALSE, warning=FALSE}
LBC.wider.summary<- mf %>% dplyr::select (FIPS, Year, STATE, COUNTY, LBC_RATE) %>%
                  tidyr::pivot_wider(names_from = Year, # column need to be wider
                  values_from = LBC_RATE) %>%
                  rowwise() %>%
                  dplyr::mutate(Mean = mean(c_across(4:18)),
...
```{r Summarise Data-1, message=FALSE, warning=FALSE}
# mean
dplyr::summarise(mf, Mean=mean(LBC_RATE))
# median
dplyr::summarise(mf, Median=median(LBC_RATE))
```
<div style="margin-bottom:20px;">
</div>
...
summary_statistics <- mf %>% 
  ...
  dplyr::select(LBC_RATE, SMOKING, POVERTY, PM25, NO2, SO2) %>%
...
```{r}
DT::datatable(as.data.frame(summary_statistics), 
...
```{r Group by Summary data, message=FALSE, warning=FALSE}
state.mf<-mf %>% 
  dplyr::group_by(STATE) %>%
  dplyr::summarize(LBC_RATE = mean(LBC_RATE),
...
```{r}
DT::datatable(as.data.frame(state.mf), 
          rownames = T, options = list(pageLength = 10, scrollX = TRUE, round)) %>%
...
```{r Glimpse Data-2, message=FALSE, warning=FALSE}
dplyr::glimpse(df.geo)
```

```{r Glimpse Data, message=FALSE, warning=FALSE}
glimpse(meta.geo)
```
<div style="margin-bottom:20px;">
</div>
...
```{r Create a Datafarme}
mf.geo <- df.geo %>%
      dplyr::select(A_LabID, StateID,  LandCover1, A_Depth, A_C_Tot, A_C_Inorg, A_C_Org, A_As, A_Cd,  A_Pb, A_Cr) %>%
      dplyr::rename("LAB_ID"=A_LabID,
...
#### **Filter Out the Data with NS**

Now, we filter out  the data where records have N.S. values INS: 

```{r Filter out NS, message=FALSE, warning=FALSE}
mf.geo<- mf.geo %>% 
        dplyr::filter(Soil_depth != "N.S." & Total_Carbon !="INS")
```

<div style="margin-bottom:20px;">
</div>

#### **Convert ND to Empty String**

Then, we will convert all N.D. values to empty string: 

```{r Convert ND to Empty String, message=FALSE, warning=FALSE}
mf.geo[mf.geo=="N.D."]<- ""
```
<div style="margin-bottom:20px;">
</div>

#### **Remove Spacial Characters and Convert Data to Double**

We have to remove **"<"** and convert the all **<chr>** to **<double>**.


```{r Remove Spacial Characters and convert data to Double, message=FALSE, warning=FALSE}
mf.geo <- mf.geo %>%
      dplyr::mutate_at(c("Arsenic", "Cadmium", "Lead", "Chromium"), str_replace, "<", "") %>%
      dplyr::mutate_at(c(5:11), as.numeric) %>% glimpse()
```
<div style="margin-bottom:20px;">
</div>

####  **Replace the Values Below Detection Limits** 

Here detection limits of As, Cd, Pb and Cr are  0.6, 0.1, 0.5, and 1 mg/kg, respectively. We will replace the  values below detection limits by half of detection limits of these heavy-metals. 


```{r Replace the  values below detection , message=FALSE, warning=FALSE}
...
```{r Omit NA, message=FALSE, warning=FALSE}
...
mf.geo.new<-mf.geo %>% dplyr::mutate_at(vars(Arsenic, Cadmium, Lead, Chromium),~ifelse(is.na(.x), mean(.x, na.rm = TRUE), .x)) %>% glimpse()
```

```{r}
mf.geo.new %>% dplyr::summarise(sum(is.na(Chromium)))
```
<div style="margin-bottom:40px;">
</div>
...
5. [Data Wrangling with Tidyverse](https://hbctraining.github.io/Intro-to-R/lessons/08_intro_tidyverse.html)
