...
* [Import Data into R](#import-data-into-r)
  ...
  - [Import Comma Delimited Text Files](#import-comma-delimited-text-files)
  
  - [Import Excel Files](#import-excel-files)
...
* [Import and Export Large Data with datatable](#import-and-export-large-data-with-datatable)
  ...
  - [Reading CSV file with fread](#reading-csv-file-with-fread)
  
  - [Writing CSV file with fwrite](#writing-csv-file-with-fwrite)


* [Feather A Fast Ondisk Format for Dataframe](#feather-a-fast-ondisk-format-for-dataframe)
...
```{r Read txt data, message=FALSE, warning=FALSE}
...
```{r Read CSV file, message=FALSE, warning=FALSE}
...
```{r Load xlsx package, message=FALSE, warning=FALSE}
# install.packages("xlsx") # Install "xlsx" package
# Load xlsx package
library(xlsx)
```

```{r Read XLX file, message=FALSE, warning=FALSE}
...
```{r Create a Dataframe, message=FALSE, warning=FALSE}
...
```{r Write as CSV file, message=FALSE, warning=FALSE}
...
```{r Write as Xlsx file, message=FALSE, warning=FALSE}
# install writexl
...
```{r Save R-Data, message=FALSE, warning=FALSE}
...
```{r Read with fread, message=FALSE, warning=FALSE}
...
```{r write with fwrite, message=FALSE, warning=FALSE}
...
```{r Reading Time, message=FALSE, warning=FALSE}
...
```{r Writing Time, message=FALSE, warning=FALSE}
...
### **Feather A Fast Ondisk Format for Dataframe**
...
```{r Load feather package, message=FALSE, warning=FALSE}
...
```{r Write as feather file, message=FALSE, warning=FALSE}
...
```{r Read feather file, message=FALSE, warning=FALSE}
...
```{r echo=FALSE, fig.align="left", fig.height=6.5, fig.width=5, out.width="70%"}
...
```{r echo=FALSE, fig.align="left", fig.height=5, fig.width=5.5, out.width="60%"}
...
```{r Read data with read_csv, message=FALSE, warning=FALSE}
...
```{r Write with write_csv, message=FALSE, warning=FALSE}
readr::write_csv(df.chem_01, "df.chem_01")
```
...
2. [R Data Import/Export](https://cran.r-project.org/doc/manuals/r-devel/R-data.htmlhttps://cran.r-project.org/doc/manuals/r-devel/R-data.html)

3. [Tidy data](https://tidyr.tidyverse.org/articles/tidy-data.html)

4. [Getting started with the tidyverse; loading your data](Getting started with the tidyverse; loading your data)

5. [Chapter 2 Importing Data in the Tidyverse](https://jhudatascience.org/tidyversecourse/get-data.html)
